...
<x-card x-data="{ open: true }" x-show="open" x-init="setTimeout(() => open = false, 5000)" {{ $attributes->merge(['class' => 'max-w-sm absolute z-100 fixed flex flex-row space-x-4 text-white ' .$alignmentClasses]) }} bgColor="{{ $bgColor }}" shadow="{{ $shadow }}" padding="px-5 py-4">
